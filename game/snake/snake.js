...
function drawSnake() {
    ...
    for(var i=0;i<snakeLength;i++) {
    	document.getElementById("tile"+snake[i][0]+"_"+snake[i][1]).classList.add('snake');
...
function initApple(){
    ...
    } while(document.getElementById("tile"+x+"_"+y).classList.contains("snake")); // 뱀이랑 겹치면 다시
...
function move() {
    ...
    // 몸통 충돌 체크
    if(document.getElementById('tile'+head[0]+'_'+head[1]).classList.contains('snake')){
        end();
        return;                    
    }
